...
class TestTokenVerification(unittest.TestCase):
    ...
    def test_verify_google_token_success(self):
        with patch("auth.google_id_token.verify_oauth2_token") as mock_verify:
    ...
    def test_verify_google_token_failure(self):
        with patch(
            "auth.google_id_token.verify_oauth2_token", side_effect=Exception("bad")
        ):
    ...
    def test_verify_firebase_token_success(self):
        with patch("auth.firebase_auth.verify_id_token") as mock_verify:
    ...
    def test_verify_firebase_token_failure(self):
        with patch("auth.firebase_auth.verify_id_token", side_effect=Exception("bad")):
            with self.assertRaises(HTTPException) as ctx:
